...
def conv_net(x, weights, biases, dropout):
    ...
    print("conv 1 before pooling: ", conv1)
   # Max Pooling (down-sampling)
    conv1 = maxpool2d(conv1, 2)
    print("Conv 1 after pooling: ", conv1)
    conv2 = conv2d(conv1, weights['wc2'], biases['bc2'])
    print("conv 2 before pooling: ", conv1)
    # Max Pooling (down-sampling)
  
    conv2 = maxpool2d(conv2, 4)
    print("conv 2 after pooling:", conv2)  
...
with tf.Session() as sess:
   ...
   # saver.save(sess, 'trained_model.sd')
    ...
    btemp=np.ones([50], dtype=np.float32)
    btemp[:] = bc2[:]
    outbytes=bytearray(btemp)
    newFile = open("./conv2.bias.bin", "wb")
    newFile.write(outbytes)
    newFile.close();
    ...
    outbytes=bytearray(wc2.flatten())
    newFile = open("./conv2.bin", "wb")
    newFile.write(outbytes)
    newFile.close();
